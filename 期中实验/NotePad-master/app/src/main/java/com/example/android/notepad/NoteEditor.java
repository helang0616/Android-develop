    ...
    protected void onPause() {
        ...
        if (mCursor != null) {
            ...
            if (isFinishing() && (length == 0)) {
                ...
                updateNote(text, null,getStandardTime());
            } else if (mState == STATE_INSERT) {
                updateNote(text, text,getStandardTime());
                mState = STATE_EDIT;
          }
        }
    }
...
//BEGIN_INCLUDE(paste)
    ...
    private final void performPaste() {
        ...
        if (clip != null) {
            ...
            if (uri != null && NotePad.Notes.CONTENT_ITEM_TYPE.equals(cr.getType(uri))) {
                ...
                if (orig != null) {
                    if (orig.moveToFirst()) {
                        int colNoteIndex = mCursor.getColumnIndex(NotePad.Notes.COLUMN_NAME_NOTE);
                        int colTitleIndex = mCursor.getColumnIndex(NotePad.Notes.COLUMN_NAME_TITLE);
                        int colModification_DateIndex = mCursor.getColumnIndex(NotePad.Notes.COLUMN_NAME_MODIFICATION_DATE);
                        text = orig.getString(colNoteIndex);
                        title = orig.getString(colTitleIndex);
                        modification_date = orig.getString(colModification_DateIndex);
                    }
            ...
            updateNote(text, title, modification_date);
        }
    }
//END_INCLUDE(paste)
     ...
     */
    private final void updateNote(String text, String title, String modification_date) {
        ...
        // This puts the date into the map.
